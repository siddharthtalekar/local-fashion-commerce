...
import { ArrowLeft, Plus, X, Upload, CheckCircle2, ChevronRight, ChevronLeft, PackageOpen, Camera, Tag, MapPin, IndianRupee, Trash2 } from 'lucide-react';
...
export default function EditProductWizard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      apiFetch<any>(`/products/retailer/${productId}`, { token })
  ...
  const handleDelete = async () => {
    ...
    try {
      await apiFetch(`/products/${productId}`, { method: 'DELETE', token: token! });
  ...
  if (loading) {
    return <div className="flex items-center justify-center p-12 text-stone-400">Loading product...</div>;
  }
    ...
    <div className="max-w-2xl mx-auto pb-8">
      ...
      <div className="flex items-center justify-between gap-4 mb-8 animate-fade-in">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black text-[#282C3F]" style={{ fontFamily: 'var(--font-display)' }}>Edit Product</h1>
            <p className="text-sm text-stone-400 mt-1">Step {step + 1} of {STEPS.length}: {STEPS[step]}</p>
          </div>
        </div>
        <button onClick={handleDelete} className="p-2.5 text-rose-500 hover:bg-rose-50 rounded-xl transition-colors border border-transparent hover:border-rose-100 flex items-center gap-2 text-sm font-bold">
      ...
      <div className="bg-white p-6 md:p-8 rounded-3xl shadow-[0_8px_40px_rgba(0,0,0,0.04)] border border-stone-100 animate-slide-up">
        ...
        <div className="flex gap-3 mt-8 pt-6 border-t border-stone-100">
          ...
          {step < STEPS.length - 1 ? (
            ...
            <button type="button" onClick={handleSubmit} disabled={saving || formData.imageUrls.filter(u => u.trim()).length === 0}
