  ...
  return (
    <div className="flex min-h-screen">
      ...
      <div className="hidden lg:flex flex-col justify-between w-[420px] flex-shrink-0 p-10 text-white relative overflow-hidden"
        ...
        <div className="absolute top-0 right-0 w-80 h-80 rounded-full blur-[120px] opacity-20"
        ...
        <div className="relative">
          ...
          <h1 className="text-3xl font-black leading-tight mb-4" style={{ fontFamily: 'var(--font-display)' }}>
            Join thousands<br />of retailers.
          </h1>
          <p className="text-white/60 text-sm leading-relaxed mb-8">
            Start selling your fashion products to local customers with zero upfront cost.
          </p>
          <div className="space-y-3">
            {perks.map((p) => (
              <div key={p} className="flex items-center gap-3 text-sm text-white/70">
                <CheckCircle2 size={16} className="text-[#FF3E6C] flex-shrink-0" />
                {p}
              </div>
            ))}
          </div>
        ...
        <div className="w-full max-w-[400px] animate-fade-in">
          ...
          <div className="bg-white rounded-3xl shadow-[0_8px_40px_rgba(0,0,0,0.08)] border border-stone-100 p-8">
            <div className="mb-8">
              ...
              <p className="text-stone-500 text-sm mt-1">Start your retailer journey today</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              {[
                { label: 'Full Name',     id: 'name',     type: 'text',  value: name,     onChange: setName,     placeholder: 'Your full name' },
                { label: 'Email',         id: 'email',    type: 'email', value: email,    onChange: setEmail,    placeholder: 'you@example.com' },
                { label: 'Phone Number',  id: 'phone',    type: 'tel',   value: phone,    onChange: setPhone,    placeholder: '9876543210' },
              ].map((f) => (
                <div key={f.id}>
                  <label className="block text-sm font-bold text-[#282C3F] mb-1.5">{f.label}</label>
                  <input
                    type={f.type}
                    value={f.value}
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full flex items-center justify-center gap-2 rounded-2xl py-3.5 text-sm font-black text-white transition-all press-effect disabled:opacity-60 mt-2"
            ...
            <p className="mt-6 text-center text-sm text-stone-500">
              ...
              <Link href="/login" className="text-[#FF3E6C] font-bold hover:underline">Sign in</Link>
            </p>
          </div>
        </div>
      </div>
